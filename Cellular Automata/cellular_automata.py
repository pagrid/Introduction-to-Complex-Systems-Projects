...
Diploid and Asynchronous Cellular Automata Simulation
-----------------------------------------------------
Author: Petros Agridos

Description:
This program implements **diploid** and **asynchronous** one-dimensional cellular automata (CA)
with probabilistic rule mixing and parallel execution. It explores how local update rules
combine under stochastic parameters (λ or α) to yield emergent macroscopic behavior.

The simulation can:
    - Encode any 8-bit elementary CA rule.
    - Apply two rules (f₁, f₂) with adjustable probability λ (diploid model).
    - Run large-scale simulations in parallel.
    - Track system density evolution over time.
    - Perform λ sweeps to compute stationary densities.
    - Extend to asynchronous updates governed by α (probability of applying f₁).

Modules:
    1. Rule Encoding
    2. Diploid Rule Application
    3. Parallel Simulation
    4. Time Evolution and Density Plots
    5. λ Sweep (Density vs λ)
    6. Asynchronous Automata
    7. Equilibrium Densities (Density vs α)

Dependencies:
    numpy, matplotlib, multiprocessing, pickle
"""
