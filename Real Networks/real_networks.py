...
Complex Network Analysis: Structural Properties and Degree Distribution Fitting
-------------------------------------------------------------------------------
Author: Petros Agridos

Description:
This program performs a **large-scale network analysis** on real-world datasets such as 
the Amazon co-purchasing network from the SNAP repository. It examines key structural 
and statistical features of complex graphs and tests whether their degree distributions 
follow **Poisson** or **power-law** behavior.

The analysis pipeline includes:
    • Automated data download and preprocessing (gzip, tab-separated files).
    • Construction of directed or undirected graphs using NetworkX.
    • Computation of adjacency matrices and clustering coefficients.
    • Visualization of degree distributions (both linear and log–log scale).
    • Statistical tests:
        - χ² goodness-of-fit for Poisson degree distribution.
        - Kolmogorov–Smirnov test for power-law fitting using the `powerlaw` package.
    • Comparison between random-like and scale-free network models.

Dependencies:
    numpy, pandas, dask[dataframe], networkx, powerlaw, matplotlib, scipy, requests

Outputs:
    • Plots:
        - Degree distribution (normal & log–log)
        - Power-law probability density fit
    • Statistical test results printed in terminal:
        - Chi-square statistic and p-value
        - KS distance for power-law fit
    • Example dataset: Amazon co-purchasing network (`amazon0302.txt.gz`)
